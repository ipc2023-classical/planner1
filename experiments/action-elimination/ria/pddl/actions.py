...
# Added 19/11/21 by Raquel Fuentetaja
    def precs(self):
        if not self.precondition.parts:
            return [self.precondition]
        return list(self.precondition.parts)

    def effs(self):
        return [eff.literal for eff in self.effects]

    def adds(self):
        return [eff.literal for eff in self.effects if not eff.literal.negated]

    def dels(self):
        return [eff.literal.negate() for eff in self.effects if eff.literal.negated ]

    def dels_negated(self):
        return [eff.literal for eff in self.effects if eff.literal.negated ]
    ...
    def strparam(self, param):
        return "%s - %s" % (param.name, param.type_name)

    
    def add_precondition(self, precondition):
        prec_list = self.precs()
        prec_list.append(precondition)
        self.precondition = conditions.Conjunction(prec_list)
    ...
    def save_pddl(self, stream, indent, task=None):
        print (indent, "(:action %s" % self.name, file = stream)
        print (indent, indent, ":parameters (%s)" % (" ".join(map(self.strparam, self.parameters))), file = stream)
        print ( indent, indent, ":precondition (and", file = stream)
        for fact in self.precs():
            if fact.negated:
                print (indent, indent, indent, indent, "(not %s)" % str(fact.positive()), file = stream)
            else:
                print (indent, indent, indent, indent, "%s" % fact, file = stream)
        print (indent, indent, indent, ")", file = stream)

        print (indent, indent, ":effect (and", file = stream)
        for eff in self.effects:
            if eff.literal.negated:
                print (indent, indent, indent, indent, "(not %s)" % eff.literal.positive(), file = stream)
            else:
                print (indent, indent, indent, indent, "%s" % eff.literal, file = stream)

        
        if "skip-original-action-plan" not in self.name and self.cost is not None and MINIMAL_COST_REDUCTION:
            numeric_constant = type(self.cost.expression) is pddl.f_expression.NumericConstant
            cost_value = "%s" if numeric_constant else "(%s)"
            print(indent, indent, indent, indent, ("(%s (%s)" + ("%s)" if numeric_constant else "(%s))")) % (self.cost.__class__.__name__.lower(), \
                re.sub(r'^.*? ', '', self.cost.fluent.__str__()).replace("()", ""), \
                re.sub(r'^.*? ', '', self.cost.expression.__str__().replace(",", " ").replace("(", " ").replace(")", ""))), file = stream)
        elif "skip-original-action-plan" in self.name or ":action-costs" in task.requirements.requirements:
            print(indent, indent, indent, indent, "(increase (total-cost) 0)", file = stream)
        else:
            print(indent, indent, indent, indent, "(increase (total-cost) 1)", file = stream)
