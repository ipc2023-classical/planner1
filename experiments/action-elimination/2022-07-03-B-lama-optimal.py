...
def _get_action_elimination_lama(**kwargs):
    # Run complete config last to guarantee that we find an optimal plan eventually.
    return [
        "--if-unit-cost",
        "--evaluator",
        "hlm=lmcount(lm_reasonable_orders_hps(lm_rhw()),pref={pref})".format(**kwargs),
...
CONFIGS = [
    ...
    ("lama-ae-hmax", _get_action_elimination_lama(pref=False, heuristic="hmax()")),
    ("lama-ae-lmcut", _get_action_elimination_lama(pref=False, heuristic="lmcut()")),
    ("lama-ae-scorpion", _get_action_elimination_lama(pref=False, heuristic="""scp_online([
        projections(sys_scp(max_time=3, max_time_per_restart=1)),
        cartesian()],
        saturator=perimstar, max_time=1000, interval=10K)""")),
]
